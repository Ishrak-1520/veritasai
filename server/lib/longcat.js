...
function repairJson(text) {
  ...
  // Step 6a — Truncate overly long string values
  cleaned = cleaned.replace(
    /"(technical_description|summary)"\s*:\s*"([^"]{300,})"/g,
    (match, key, value) => {
      // Take first 280 chars, end at last complete word
      const truncated = value.substring(0, 280).replace(/\s+\S*$/, '');
...
async function callLongcatOpenAI(body, label) {
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      ...
      require('fs').writeFileSync('raw_content.json', content, 'utf8');
